  ...
  <head>
    ...
    <title>Авторизация</title>
      ...
      <form method="post" align="center" class="regform">
        ...
        <label>Имя *</label><br>
        <input type="text" name="surname" required value="<? if(isset($_POST['surname'])) echo $_POST['name']; ?>"><?php if(isset($nam)) echo $nam;?><br>
        <label>Фамилия *</label><br>
        <input type="text" name="name" required value="<? if(isset($_POST['name'])) echo $_POST['surname']; ?>"><?php if(isset($surnname)) echo $surnname;?><br>
         ...
         && !empty($_POST['sex']) && !empty($_POST['email']) && !empty($_POST['age']) && !empty($_POST['country']))
        {
                ...
                if(!preg_match('/^[A-Za-z0-9@!.#]{5,}+$/i',$_POST['pwd']))
            {
               echo '<span>Неправильно задан пароль.<br>
                          Используйте только английские буквы и эти символы @!.#.<br>
